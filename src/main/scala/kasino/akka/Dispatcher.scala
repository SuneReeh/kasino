...
def dispatch[T <: Message]( recipient : ActorRef[Letter[T]], message: T)(context: ActorContext[?]) : ActorRef[Try[Done]] = context.spawn(Behaviors.setup(context => new Dispatcher[T](recipient, message, context)),"Dispatcher")
